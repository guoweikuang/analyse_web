...
from .get_mysql_content import especial_using1
...
class Classify(object):
    def __init__(self, filename, time, days):
        self.rows = get_rows(time, days)
        self.seg = []
        self.filename = filename

    def save_to_one_file(self, name, contents, comment_num, zan_num, public_time):
        with open(self.filename + '/{}.txt'.format(name), 'ab') as fp:

            fp.write(contents.encode('utf-8') + '\t'.encode('utf-8') + comment_num.encode('utf-8')
...
def topic(start_time, days):
    classify_topic = Classify(abs_filename + '/分类结果', start_time, days)
